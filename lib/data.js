...
export const STORAGE_KEY = "megs-dashboard-v1";
...
export function assignmentAverage(cls, assignmentId) {
  const a = cls.assignments.find((x) => x.id === assignmentId);
  if (!a || !Number(a.points)) return null;
  let sum = 0;
  let count = 0;
  for (const s of cls.students) {
    const v = cls.scores?.[s.id]?.[assignmentId];
    if (v === null || v === undefined || v === "") continue;
    const n = Number(v);
    if (Number.isNaN(n)) continue;
    sum += (n / Number(a.points)) * 100;
    count++;
  }
