...
export async function generateTrackingDocx(kind, students, className) {
  ...
  let filename;
  if (kind === "workbook") {
    // every document starts on an odd page (a fresh sheet front), so each
    // one can be pulled out or duplex-printed without bleeding into the
    // back of the previous document
    const sections = [r.coverSection(specs)];
    for (const s of specs) sections.push(r.templateSection(s, D.SectionType.ODD_PAGE));
    doc = new D.Document({
...
function renderer(D, className) {
  ...
  const templateSection = (spec, sectionType) => ({
    properties: { ...sectionProps(spec.landscape), type: sectionType },
