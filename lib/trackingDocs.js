...
const FONT = "Calibri";
const SERIF = "Special Elite"; // farmhouse typewriter (built into Google Docs)
...
export async function generateTrackingDocx(kind, students, className) {
  ...
  if (kind === "workbook") {
    ...
    filename = `Student Tracking Documents${className ? ` - ${className}` : ""}.docx`;
  } else {
    ...
    filename = `${spec.file}${className ? ` - ${className}` : ""}.docx`;
  }
